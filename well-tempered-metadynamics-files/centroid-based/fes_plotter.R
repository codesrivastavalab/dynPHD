...
p<-plot_ly(x=fes[,1],y=fes[,2],z=fes[,3]/4.184,type="contour",autocontour = T,
  ...
  layout(title = "",titlefont=list(size=30,family="Droid Sans"),
         xaxis = list(tickmode="auto",nticks=10,title = "CV-1 (nm)",tickfont=list(size=20),titlefont=list(size=30,family="Droid Sans"),range=c(1.2,2.8)),
         yaxis = list(tickmode="auto",nticks=10,title = "CV-2 (rad)",tickfont=list(size=20),titlefont=list(size=30,family="Droid Sans"),range=c(0,3.14)))
p		
